...
<body>
				...
				<tbody class="pn-ltbody">
					...
					<c:forEach items="${order.orderItemList }" var="orderItem">
						<tr style="padding-top: 20px; padding-bottom: 20px;">
							...
							<td align="center">${orderItem.book.bid }</td>
							<td align="center">
	...
	<div style="margin-top:15px;">
		...
		<input id="deliver" type="button" onclick="javascript:window.location.href='<c:url value="/admin/AdminOrderServlet?method=deliver&oid=${order.oid }"/>'" value="发货" class="submit"/>
		</c:if>
		<c:if test="${order.status eq 1 and btn eq 'cancel' }">
		<input id="cancel" type="button" onclick="javascript:window.location.href='<c:url value="/admin/AdminOrderServlet?method=cancel&oid=${order.oid }"/>'" value="取消" class="submit"/>
		</c:if>
	</div>
</body>
</html>