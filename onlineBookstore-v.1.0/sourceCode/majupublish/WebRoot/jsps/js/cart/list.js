	...
	$(".jian").click(function(){
		...
		if(quantity==1){
			if(confirm("您是否真确认该删除该条目？")){
				location="/majupublish/CartItemServlet?method=batchDelete&cartItemIds="+id;
			}
		}else{
...
function _batchDelete(){
		...
		if(cartItemIds.length!=0){
			if(confirm("您确定要删除这些条目？")){
				location="/majupublish/CartItemServlet?method=batchDelete&cartItemIds="+cartItemIds.toString();
			}
		}else{
			alert("您还没有选中条目！");
		}
